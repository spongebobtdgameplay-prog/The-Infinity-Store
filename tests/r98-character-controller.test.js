...
test('R98 removes stacked movement, contact, and renderer wrappers', () => {
  ...
  for (const pattern of [/movement-hard-stop\.js/, /runtime-collision-supplement-r97\.js/, /runtime-character-physics-r97\.js/]) assert.doesNotMatch(index, pattern);
  for (const pattern of [/movement-contact-compat-r25\.js/, /final-contact-r19\.js/]) assert.doesNotMatch(bootstrap, pattern);
  for (const file of ['movement-hard-stop.js','runtime-collision-supplement-r97.js','runtime-character-physics-r97.js','movement-contact-compat-r25.js','final-contact-r19.js']) assert.equal(exists(file), false, `${file} should be removed`);
});
