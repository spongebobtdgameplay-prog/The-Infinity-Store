...
const Build = "V0.35.66-R95-ENGINE";
...
class GroupedSpringDeformer {
  ...
  BuildGroups() {
    ...
    for (let Group = 0; Group < this.GroupCount; Group += 1) {
      ...
      NY /= Length;
      NZ /= Length;
      this.GroupNormal[Offset] = NX;
      this.GroupNormal[Offset + 1] = NY;
      this.GroupNormal[Offset + 2] = NZ;
    }
  }
  ...
  Step(Delta) {
    ...
    for (let Group = 0; Group < this.GroupCount; Group += 1) {
      ...
      let VX = (this.GroupVelocity[Offset] + Dt * Spring * (TargetX - this.GroupDisplacement[Offset])) * DampingFactor;
      let VY = (this.GroupVelocity[Offset + 1] + Dt * Spring * (TargetY - this.GroupDisplacement[Offset + 1])) * DampingFactor;
      let VZ = (this.GroupVelocity[Offset + 2] + Dt * Spring * (TargetZ - this.GroupDisplacement[Offset + 2])) * DampingFactor;
      ...
      if (VX * VX + VY * VY + VZ * VZ > ThresholdSquared || X * X + Y * Y + Z * Z > ThresholdSquared) {
        Moving = true;
      }
    }
  ...
  Apply() {
    ...
    this.Frame += 1;
    if (this.Options.RecalculateNormals && this.Frame % this.Options.NormalUpdateEvery === 0) {
...
function UnregisterDeformer(Deformer) {
  ...
  if (Deformer.Mesh?.userData?.GroupedSpringDeformerR95 === Deformer) {
    delete Deformer.Mesh.userData.GroupedSpringDeformerR95;
  }
  return true;
}
